...
##TheyWereVeryEasy_WoP Survey III vs WP Survey III
# Filter data for WP group and 'TheyWereVeryEasy' column only for Surveys III
Survey_III_Data <- Data %>% 
  filter(Survey == "III" & (Group == "WoP" | Group == "WP")) %>% 
  select(Survey, Group, TheyWereVeryEasy)

# Calculate mean, median, and standard deviation for 'TheyWereVeryEasy' within WoP group
summary_stats <- Survey_III_Data %>%
  group_by(Group) %>%
  summarize(Mean = mean(TheyWereVeryEasy, na.rm = TRUE),
            Median = median(TheyWereVeryEasy, na.rm = TRUE),
            SD = sd(TheyWereVeryEasy, na.rm = TRUE))
...
Survey_III_Data <- Survey_III_Data %>%
  mutate(mean = ifelse(Group == "WoP", M1, M2),
         Residual = TheyWereVeryEasy - mean)

# Filter out NA values for 'TheyWereVeryEasy'
valid_data <- Survey_III_Data %>%
  filter(!is.na(TheyWereVeryEasy))

# Extract data for Wilcoxon test
x <- valid_data$TheyWereVeryEasy[valid_data$Group == "WoP"]
y <- valid_data$TheyWereVeryEasy[valid_data$Group == "WP"]
...
write.csv(results, file = "TheyWereVeryEasy_Survey III WoP vs WP.csv", row.names = FALSE)
warnings()