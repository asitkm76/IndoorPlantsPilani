...
# Filter data for WoP group and 'P' column only for Surveys II and IV
WoP_Data <- Data %>% 
  filter(Group == "WoP" & (Survey == "II" | Survey == "IV")) %>% 
  select(Group, Survey, P)

# Calculate mean, median, and standard deviation for 'P' within WoP group
summary_stats <- WoP_Data %>%
  group_by(Survey) %>%
  summarize(Mean = mean(P, na.rm = TRUE),
...
# Filter out NA values for 'P'
valid_data <- WoP_Data %>%
  filter(!is.na(P))

# Extract data for Wilcoxon test
x <- valid_data$P[valid_data$Survey == "II"]
y <- valid_data$P[valid_data$Survey == "IV"]
...
write.csv(results1, file = "P_WoP_Survey_II_vs_IV.csv", row.names = FALSE)
warnings()

######
##P_WP_Survey_II_vs_IV
# Filter data for WP group and 'P' column only for Surveys II and IV
WP_Data <- Data %>% 
...
# Save the results to a CSV file
write.csv(results2, file = "P_WP_Survey_II_vs_IV.csv", row.names = FALSE)
warnings()


######
##P_WoP Survey IV vs WP Survey IV
# Filter data for WP group and 'P' column only for Surveys IV
Survey_IV_Data <- Data %>% 
  filter(Survey == "IV" & (Group == "WoP" | Group == "WP")) %>% 
  select(Survey, Group, P)
...
# Extract data for Wilcoxon test
x <- valid_data$P[valid_data$Group == "WoP"]
y <- valid_data$P[valid_data$Group == "WP"]
...
# Save the results to a CSV file
write.csv(results3, file = "P_Survey IV WoP vs WP.csv", row.names = FALSE)
warnings()