...
# Filter data for WoP group and 'TempSat' column only for Surveys II and IV
WoP_Data <- Data %>% 
  filter(Group == "WoP" & (Survey == "II" | Survey == "IV")) %>% 
  select(Group, Survey, TempSat)

# Calculate mean, median, and standard deviation for 'TempSat' within WoP group
summary_stats <- WoP_Data %>%
  group_by(Survey) %>%
  summarize(Mean = mean(TempSat, na.rm = TRUE),
            Median = median(TempSat, na.rm = TRUE),
            SD = sd(TempSat, na.rm = TRUE))
...
WoP_Data <- WoP_Data %>%
  mutate(mean = ifelse(Survey == "II", M1, M2),
         Residual = TempSat - mean)

# Filter out NA values for 'TempSat'
valid_data <- WoP_Data %>%
  filter(!is.na(TempSat))

# Extract data for Wilcoxon test
x <- valid_data$TempSat[valid_data$Survey == "II"]
y <- valid_data$TempSat[valid_data$Survey == "IV"]
...
write.csv(results1, file = "TempSat_WoP_Survey_II_vs_IV.csv", row.names = FALSE)

######
##TempSat_WP_Survey_II_vs_IV
# Filter data for WP group and 'TempSat' column only for Surveys II and IV
WP_Data <- Data %>% 
...
# Save the results to a CSV file
write.csv(results2, file = "TempSat_WP_Survey_II_vs_IV.csv", row.names = FALSE)


######
##TempSat_WoP Survey IV vs WP Survey IV
# Filter data for WP group and 'TempSat' column only for Surveys IV
Survey_IV_Data <- Data %>% 
  filter(Survey == "IV" & (Group == "WoP" | Group == "WP")) %>% 
  select(Survey, Group, TempSat)
...
# Extract data for Wilcoxon test
x <- valid_data$TempSat[valid_data$Group == "WoP"]
y <- valid_data$TempSat[valid_data$Group == "WP"]
...
# Save the results to a CSV file
write.csv(results3, file = "TempSat_Survey IV WoP vs WP.csv", row.names = FALSE)
