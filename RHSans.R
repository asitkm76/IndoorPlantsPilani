...
# Filter data for WoP group and 'RHSans' column only for Surveys II and IV
WoP_Data <- Data %>% 
  filter(Group == "WoP" & (Survey == "II" | Survey == "IV")) %>% 
  select(Group, Survey, RHSans)

# Calculate mean, median, and standard deviation for 'RHSans' within WoP group
summary_stats <- WoP_Data %>%
  group_by(Survey) %>%
  summarize(Mean = mean(RHSans, na.rm = TRUE),
            Median = median(RHSans, na.rm = TRUE),
            SD = sd(RHSans, na.rm = TRUE))
...
WoP_Data <- WoP_Data %>%
  mutate(mean = ifelse(Survey == "II", M1, M2),
         Residual = RHSans - mean)

# Filter out NA values for 'RHSans'
valid_data <- WoP_Data %>%
  filter(!is.na(RHSans))

# Extract data for Wilcoxon test
x <- valid_data$RHSans[valid_data$Survey == "II"]
y <- valid_data$RHSans[valid_data$Survey == "IV"]

# Perform paired Wilcoxon test if both groups have data
if (length(x) > 0 && length(y) > 0 && length(x) == length(y)) {
  wilcox_test <- wilcox.test(x, y, distribution = "asymptotic", alternative = "two.sided", paired = TRUE)
...
write.csv(results1, file = "RHSans_WoP_Survey_II_vs_IV.csv", row.names = FALSE)
warnings()

######
##RHSans_WP_Survey_II_vs_IV
# Filter data for WP group and 'RHSans' column only for Surveys II and IV
WP_Data <- Data %>% 
...
# Save the results to a CSV file
write.csv(results2, file = "RHSans_WP_Survey_II_vs_IV.csv", row.names = FALSE)
warnings()


######
##RHSans_WoP Survey IV vs WP Survey IV
# Filter data for WP group and 'RHSans' column only for Surveys IV
Survey_IV_Data <- Data %>% 
  filter(Survey == "IV" & (Group == "WoP" | Group == "WP")) %>% 
  select(Survey, Group, RHSans)
...
# Extract data for Wilcoxon test
x <- valid_data$RHSans[valid_data$Group == "WoP"]
y <- valid_data$RHSans[valid_data$Group == "WP"]
...
# Save the results to a CSV file
write.csv(results3, file = "RHSans_Survey IV WoP vs WP.csv", row.names = FALSE)
warnings()