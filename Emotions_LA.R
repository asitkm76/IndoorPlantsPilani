...
# Filter data for WoP group and 'LA' column only for Surveys II and IV
WoP_Data <- Data %>% 
  filter(Group == "WoP" & (Survey == "II" | Survey == "IV")) %>% 
  select(Group, Survey, LA)

# Calculate mean, median, and standard deviation for 'LA' within WoP group
summary_stats <- WoP_Data %>%
  group_by(Survey) %>%
  summarize(Mean = mean(LA, na.rm = TRUE),
            Median = median(LA, na.rm = TRUE),
            SD = sd(LA, na.rm = TRUE))
...
# Filter out NA values for 'LA'
valid_data <- WoP_Data %>%
  filter(!is.na(LA))

# Extract data for Wilcoxon test
x <- valid_data$LA[valid_data$Survey == "II"]
y <- valid_data$LA[valid_data$Survey == "IV"]
...
write.csv(results1, file = "LA_WoP_Survey_II_vs_IV.csv", row.names = FALSE)
warnings()

######
##LA_WP_Survey_II_vs_IV
# Filter data for WP group and 'LA' column only for Surveys II and IV
WP_Data <- Data %>% 
...
# Save the results to a CSV file
write.csv(results2, file = "LA_WP_Survey_II_vs_IV.csv", row.names = FALSE)
warnings()


######
##LA_WoP Survey IV vs WP Survey IV
# Filter data for WP group and 'LA' column only for Surveys IV
Survey_IV_Data <- Data %>% 
  filter(Survey == "IV" & (Group == "WoP" | Group == "WP")) %>% 
  select(Survey, Group, LA)
...
# Extract data for Wilcoxon test
x <- valid_data$LA[valid_data$Group == "WoP"]
y <- valid_data$LA[valid_data$Group == "WP"]
...
# Save the results to a CSV file
write.csv(results3, file = "LA_Survey IV WoP vs WP.csv", row.names = FALSE)
