...
column_names <- c('Extraversion', 'Agreeableness', 'Conscientiousness', 'Neuroticism', 
...
for (column_name in column_names) {
  ...
  if (length(x) > 0 && length(y) > 0 && length(x) == length(y)) {
    wilcox_test <- wilcox.test(x, y, distribution = "asymptotic",alternative = "two.sided", paired = FALSE)
...
write.csv(results, file = "Personality.csv", row.names = FALSE)
 warnings()