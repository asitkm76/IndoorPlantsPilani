...
# Filter data for WoP group and 'TempSans' column only for Surveys II and IV
WoP_Data <- Data %>% 
  filter(Group == "WoP" & (Survey == "II" | Survey == "IV")) %>% 
  select(Group, Survey, TempSans)

# Calculate mean, median, and standard deviation for 'TempSans' within WoP group
summary_stats <- WoP_Data %>%
  group_by(Survey) %>%
  summarize(Mean = mean(TempSans, na.rm = TRUE),
            Median = median(TempSans, na.rm = TRUE),
            SD = sd(TempSans, na.rm = TRUE))
...
WoP_Data <- WoP_Data %>%
  mutate(mean = ifelse(Survey == "II", M1, M2),
         Residual = TempSans - mean)

# Filter out NA values for 'TempSans'
valid_data <- WoP_Data %>%
  filter(!is.na(TempSans))

# Extract data for Wilcoxon test
x <- valid_data$TempSans[valid_data$Survey == "II"]
y <- valid_data$TempSans[valid_data$Survey == "IV"]
...
write.csv(results1, file = "TempSans_WoP_Survey_II_vs_IV.csv", row.names = FALSE)

######
##TempSans_WP_Survey_II_vs_IV
# Filter data for WP group and 'TempSans' column only for Surveys II and IV
WP_Data <- Data %>% 
...
# Save the results to a CSV file
write.csv(results2, file = "TempSans_WP_Survey_II_vs_IV.csv", row.names = FALSE)


######
##TempSans_WoP Survey IV vs WP Survey IV
# Filter data for WP group and 'TempSans' column only for Surveys IV
Survey_IV_Data <- Data %>% 
  filter(Survey == "IV" & (Group == "WoP" | Group == "WP")) %>% 
  select(Survey, Group, TempSans)
...
# Extract data for Wilcoxon test
x <- valid_data$TempSans[valid_data$Group == "WoP"]
y <- valid_data$TempSans[valid_data$Group == "WP"]
...
# Save the results to a CSV file
write.csv(results3, file = "TempSans_Survey IV WoP vs WP.csv", row.names = FALSE)
