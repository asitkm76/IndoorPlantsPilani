...
##NoTimePressure_WoP Survey III vs WP Survey III
# Filter data for WP group and 'NoTimePressure' column only for Surveys III
Survey_III_Data <- Data %>% 
  filter(Survey == "III" & (Group == "WoP" | Group == "WP")) %>% 
  select(Survey, Group, NoTimePressure)

# Calculate mean, median, and standard deviation for 'NoTimePressure' within WoP group
summary_stats <- Survey_III_Data %>%
  group_by(Group) %>%
  summarize(Mean = mean(NoTimePressure, na.rm = TRUE),
            Median = median(NoTimePressure, na.rm = TRUE),
            SD = sd(NoTimePressure, na.rm = TRUE))
...
Survey_III_Data <- Survey_III_Data %>%
  mutate(mean = ifelse(Group == "WoP", M1, M2),
         Residual = NoTimePressure - mean)

# Filter out NA values for 'NoTimePressure'
valid_data <- Survey_III_Data %>%
  filter(!is.na(NoTimePressure))

# Extract data for Wilcoxon test
x <- valid_data$NoTimePressure[valid_data$Group == "WoP"]
y <- valid_data$NoTimePressure[valid_data$Group == "WP"]
...
write.csv(results, file = "NoTimePressure_Survey III WoP vs WP.csv", row.names = FALSE)
