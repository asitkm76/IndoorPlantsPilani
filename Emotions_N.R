...
# Filter data for WoP group and 'N' column only for Surveys II and IV
WoP_Data <- Data %>% 
  filter(Group == "WoP" & (Survey == "II" | Survey == "IV")) %>% 
  select(Group, Survey, N)

# Calculate mean, median, and standard deviation for 'N' within WoP group
...
# Filter out NA values for 'N'
valid_data <- WoP_Data %>%
  filter(!is.na(N))

# Extract data for Wilcoxon test
x <- valid_data$N[valid_data$Survey == "II"]
y <- valid_data$N[valid_data$Survey == "IV"]
...
write.csv(results1, file = "N_WoP_Survey_II_vs_IV.csv", row.names = FALSE)

######
##N_WP_Survey_II_vs_IV
# Filter data for WP group and 'N' column only for Surveys II and IV
WP_Data <- Data %>% 
...
# Save the results to a CSV file
write.csv(results2, file = "N_WP_Survey_II_vs_IV.csv", row.names = FALSE)


######
##N_WoP Survey IV vs WP Survey IV
# Filter data for WP group and 'N' column only for Surveys IV
Survey_IV_Data <- Data %>% 
  filter(Survey == "IV" & (Group == "WoP" | Group == "WP")) %>% 
  select(Survey, Group, N)
...
# Extract data for Wilcoxon test
x <- valid_data$N[valid_data$Group == "WoP"]
y <- valid_data$N[valid_data$Group == "WP"]
...
# Save the results to a CSV file
write.csv(results3, file = "N_Survey IV WoP vs WP.csv", row.names = FALSE)
