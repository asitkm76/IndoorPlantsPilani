...
##WorkedAtZeropercentOfCapacity_WoP Survey III vs WP Survey III
# Filter data for WP group and 'WorkedAtZeropercentOfCapacity' column only for Surveys III
Survey_III_Data <- Data %>% 
  filter(Survey == "III" & (Group == "WoP" | Group == "WP")) %>% 
  select(Survey, Group, WorkedAtZeropercentOfCapacity)

# Calculate mean, median, and standard deviation for 'WorkedAtZeropercentOfCapacity' within WoP group
summary_stats <- Survey_III_Data %>%
  group_by(Group) %>%
  summarize(Mean = mean(WorkedAtZeropercentOfCapacity, na.rm = TRUE),
            Median = median(WorkedAtZeropercentOfCapacity, na.rm = TRUE),
            SD = sd(WorkedAtZeropercentOfCapacity, na.rm = TRUE))
...
Survey_III_Data <- Survey_III_Data %>%
  mutate(mean = ifelse(Group == "WoP", M1, M2),
         Residual = WorkedAtZeropercentOfCapacity - mean)

# Filter out NA values for 'WorkedAtZeropercentOfCapacity'
valid_data <- Survey_III_Data %>%
  filter(!is.na(WorkedAtZeropercentOfCapacity))

# Extract data for Wilcoxon test
x <- valid_data$WorkedAtZeropercentOfCapacity[valid_data$Group == "WoP"]
y <- valid_data$WorkedAtZeropercentOfCapacity[valid_data$Group == "WP"]
...
if (length(x) > 0 && length(y) > 0 && length(x) == length(y)) {
  ...
  results <- data.frame(
    Column = 'WorkedAtZeropercentOfCapacity',
    Group = c("WoP", "WP"),
...
write.csv(results, file = "WorkedAtZeropercentOfCapacity_Survey III WoP vs WP.csv", row.names = FALSE)
warnings()